...
<div class={`@container @lg:rounded-xl shadow-md flex flex-col overflow-hidden group ${extraClasses}`} {...restProps}>
    <a href="{link}" class="flex-grow @lg:hover:bg-neutral-800 @lg:p-4">
        <div class="flex flex-col-reverse @xl:flex-row @xl:items-start">
            {#if image}
                <img 
                    ...
                    alt="{title}" 
                    loading="lazy"
                    class="aspect-video w-4/5 max-w-12 rounded-sm border border-2 border-neutral-400 group-hover:border-neutral-200 object-cover @xl:mr-4 @xl:w-1/4" 
                />
            {/if}
